    ...
    historyEl.innerHTML = history.map(h => `
      <div class="history-item">
        <div class="history-header">
          ...
          <span class="timestamp">${new Date(h.debut_statut).toLocaleTimeString('fr-CA', { hour: '2-digit', minute: '2-digit' })}</span>
        </div>
    ...
    return `
      <div class="piece-item">
        <div>
          ...
          <div class="piece-info">${new Date(p.updated_at).toLocaleDateString('fr-CA')}</div>
        </div>
    ...
    listEl.innerHTML = expeditions.map(e => `
      <div class="history-item">
        <div class="history-header">
          ...
          <span class="timestamp">${new Date(e.expedie_le).toLocaleDateString('fr-CA')}</span>
        </div>
...
function toggleAPEXManual() {
  ...
  if (row.style.display !== 'none') {
    document.getElementById('apex-manual-input').focus();
  }
}

// Save manual input
function saveAPEXManual() {
  const v = document.getElementById('apex-manual-input').value.trim();
  if (!v || !apexActivePosition) return;
  saveAPEXValue(v);
  document.getElementById('apex-manual-input').value = '';
  document.getElementById('apex-manual-row').style.display = 'none';
}
...
async function saveAPEXValue(value) {
  ...
  try {
    ...
    if (!piece) {
      if (!confirm(`Pièce inconnue : ${value}. La créer avec le statut "Inventaire - Prêt" ?`)) {
        statusEl.textContent = '❌ Opération annulée';
        statusEl.className = 'apex-status err';
        return;
      }
      piece = await sbInsert('pieces', { no_piece: value, statut: 'Inventaire - Prêt' });
      await enregistrerAudit({
        typeEntite: 'pieces',
        entiteId: piece.id,
        action: 'creation',
    ...
    // Check if piece can be installed
    if (piece.statut !== 'Inventaire - Prêt') {
      if (!confirm(`La pièce ${piece.no_piece} est au statut "${piece.statut}". Forcer l'installation ?`)) {
        statusEl.textContent = '❌ Opération annulée';
        statusEl.className = 'apex-status err';
        return;
      }
